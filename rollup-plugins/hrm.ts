...
const BACKGROUND_FILE = resolve(
  __dirname,
  "../src/background/service-worker.ts"
);
const SRC_DIR = resolve(__dirname, "../src");

function debounce(fn: (...args: any[]) => void, timeout = 400) {
...
export const HRMMiddleware = () => {
  ...
  const handleFileChange = debounce((event, filePath) => {
    ...
    // 处理background
    if (filePath === BACKGROUND_FILE || filePath.includes("/background/")) {
      console.log("Background script changed, reloading extension");
      send("reload-app");
      return;
    }
    ...
    if (filePath.includes("\\src\\")) {
      ...
      // 对于某些特定文件，可能需要完全重新加载
      if (filePath.includes("App.vue") || filePath.includes("main.ts")) {
